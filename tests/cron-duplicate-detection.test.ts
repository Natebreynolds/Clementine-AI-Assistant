...
describe('findDuplicateJobDefinitions', () => {
  it('returns empty when no global job has agentSlug', () => {
    ...
    const agents = [makeJob({ name: 'sales-agent:c', agentSlug: 'sales-agent' })];
  ...
  it('returns empty when global agentSlug job has no matching agent definition', () => {
    const global = [makeJob({ name: 'a', agentSlug: 'sales-agent' })];
    const agents = [makeJob({ name: 'sales-agent:b', agentSlug: 'sales-agent' })];
    expect(findDuplicateJobDefinitions(global, agents)).toEqual([]);
  });
  ...
  it('flags same bare name in global+agent and reports diverged fields', () => {
    ...
    const agents = [makeJob({
      name: 'sales-agent:market-leader-followup',
      agentSlug: 'sales-agent',
      schedule: '30 8 * * *',
    ...
    expect(dupes[0]!.agentSlug).toBe('sales-agent');
  ...
  it('reports zero diverged fields when the two definitions are identical', () => {
    ...
    const global = [makeJob({ name: 'sync', agentSlug: 'marketing-agent', ...shared })];
    const agents = [makeJob({ name: 'marketing-agent:sync', agentSlug: 'marketing-agent', ...shared })];
    const dupes = findDuplicateJobDefinitions(global, agents);
    expect(dupes).toHaveLength(1);
  ...
  it('handles multiple duplicates across different agents', () => {
    const global = [
      makeJob({ name: 'a', agentSlug: 'sales-agent' }),
      makeJob({ name: 'b', agentSlug: 'marketing-agent' }),
    ];
    const agents = [
      makeJob({ name: 'sales-agent:a', agentSlug: 'sales-agent' }),
      makeJob({ name: 'marketing-agent:b', agentSlug: 'marketing-agent' }),
    ];
    const dupes = findDuplicateJobDefinitions(global, agents);
    expect(dupes).toHaveLength(2);
    expect(dupes.map(d => d.agentSlug).sort()).toEqual(['marketing-agent', 'sales-agent']);
  });
});
