...
server.tool(
  ...
  async ({ action, content, section, file_path, salience_hint, reason, supersedes_chunk_id }) => {
    ...
    if (action === 'update_identity') {
      ...
      try {
        ...
        store.logExtraction({
          sessionKey: 'mcp', userMessage: content.slice(0, 200),
          toolName: 'memory_write',
          toolInput: JSON.stringify({ action, reason, salience_hint: effectiveHint }),
          extractedAt: new Date().toISOString(), status: 'active',
          agentSlug: ACTIVE_AGENT_SLUG ?? undefined,
        });
      } catch { /* observability is best-effort */ }
    ...
    if (action === 'write_note') {
      ...
      try {
        ...
        store.logExtraction({
          sessionKey: 'mcp', userMessage: content.slice(0, 200),
          toolName: 'memory_write',
          toolInput: JSON.stringify({ action, file_path: relPath, reason, salience_hint }),
          extractedAt: new Date().toISOString(), status: 'active',
          agentSlug: ACTIVE_AGENT_SLUG ?? undefined,
        });
      } catch { /* observability is best-effort */ }
    ...
    if (action === 'supersede') {
      ...
      try {
        store.logExtraction({
          sessionKey: 'mcp', userMessage: content.slice(0, 200),
          toolName: 'memory_write',
          toolInput: JSON.stringify({ action, supersedes_chunk_id, new_chunk_id: newChunkId, reason, salience_hint, section: targetSection }),
